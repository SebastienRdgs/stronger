...
const input = `Game 1: 12 blue; 2 green, 13 blue, 19 red; 13 red, 3 green, 14 blue
...
const solve = (input: string) => {
  ...
  const validGames = games.map((v: string, i: number) => {
    const records = v.split(`Game ${i + 1}: `);
    const sets = records[1].split(";");
    const validSet = sets.map((v: string, i: number) => {
      const cubes = v.trim().split(",");
      const each = cubes.map((v: string, i: number) => {
        const cube = v.trim().split(" ");
        const check = checkCube(cube);
        return check;
      })
      return each.find((c) => c == false)
    })
    return validSet.find((c) => c == false)
  });
  const finalRes = validGames.reduce((acc: number, c: boolean, i: number) => {
    if (c !== false) acc += i + 1;
    return acc;
  }, 0)
  return finalRes;
...
const checkCube = (cube: Array<string>) => {
  const maxCubes = {
    red: 12,
    green: 13,
    blue: 14
  ...
  let res = true;
  if (cube[1] === 'red' && parseInt(cube[0]) > maxCubes.red) {
    res = false;
  } else if (cube[1] === 'green' && parseInt(cube[0]) > maxCubes.green) {
    res = false;
  } else if (cube[1] === 'blue' && parseInt(cube[0]) > maxCubes.blue) {
    res = false;
  }
