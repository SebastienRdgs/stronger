...
const solve = (input: string) => {
  ...
  const powers = games.map((v: string, i: number) => {
    const records = v.split(`Game ${i + 1}: `);
    const sets = records[1].split(";");
    const eachSets = sets.map((v: string, i: number) => {
      const cubes = v.trim().split(",");
      const eachSet = cubes.map((v: string, i: number) => {
        const cube = v.trim().split(" ");
        return cube;
      })
      return eachSet;
    });
    const r = checkSets(eachSets);
    return r.blue * r.green * r.red;
  });

  const finalRes = powers.reduce((acc: number, c: number) => acc + c, 0)
  return finalRes;
}

const checkSets = (sets: string[][][]) => {
  const miniCubeSet = {
    red: 0,
    green: 0,
    blue: 0
  ...
  for (const set of sets) {
    for (const s of set) {
      if (miniCubeSet[s[1]] < parseInt(s[0])) {
        miniCubeSet[s[1]] = parseInt(s[0]);
      }
    }
  }
  return miniCubeSet;
